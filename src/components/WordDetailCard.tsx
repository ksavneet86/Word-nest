        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {!isSynAnt && w.pos && <Badge color={POS_COLORS[w.pos] || color}>{w.pos}</Badge>}
              {!isSynAnt && w.difficulty && <Badge color="#94A3B8">{w.difficulty}</Badge>}
              {!isSynAnt && w.category && w.category !== "other" && <Badge color="#38BDF8">{w.category}</Badge>}
            </div>
            {!isSynAnt && <p className="text-slate-600 mt-1">{w.meaning}</p>}
          </div>
        </div>
